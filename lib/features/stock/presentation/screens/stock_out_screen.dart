/// Stock Out Screen — Full screen for recording stock OUT entries.
...
class StockOutScreen extends ConsumerStatefulWidget {
  ...
  const StockOutScreen({super.key, required this.productId});
...
class _StockOutScreenState extends ConsumerState<StockOutScreen> {
  ...
  void initState() {
    super.initState();
    // Pre-fill rate from product's sale price
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final products = ref.read(productsProvider).valueOrNull ?? [];
      final product = products.where((p) => p.id == widget.productId).firstOrNull;
      if (product != null) {
        _rateController.text = product.salePrice.toStringAsFixed(0);
      }
    });
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (product != null && qty > product.currentStock) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'موجودہ اسٹاک (${product.currentStock.toStringAsFixed(0)} ${product.unit}) سے زیادہ مقدار نہیں نکال سکتے',
      ...
      await ref
          ...
          .addStockEntry(
            entryType: StockType.stockOut,
            quantity: qty,
            rate: double.tryParse(_rateController.text) ?? 0,
            note: _customerController.text.trim().isNotEmpty
                ? '${_customerController.text.trim()}${_noteController.text.trim().isNotEmpty ? ' - ${_noteController.text.trim()}' : ''}'
        ...
        child: Column(
          ...
          if (product != null && product.currentStock <= 0)
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: AppColors.debit.withOpacity(0.3)),
              ),
              child: Row(
                children: [
                  const Icon(Icons.warning_amber_rounded, color: AppColors.debit, size: 20),
                  const SizedBox(width: 8),
                  const Expanded(
                    child: Text(
                      '⚠️ اسٹاک ختم ہو چکا ہے!',
                      style: TextStyle(
                        fontFamily: AppTextStyles.urduFont,
                        fontSize: 13,
                        fontWeight: FontWeight.bold,
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: AppColors.debit.withOpacity(0.2)),
              boxShadow: [
                BoxShadow(
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(Icons.arrow_upward, color: AppColors.debit, size: 24),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            if (product != null)
                              Text(
                                'دستیاب: ${product.currentStock.toStringAsFixed(0)} ${product.unit}',
                  ...
                  if (_qtyController.text.isNotEmpty && _rateController.text.isNotEmpty)
                    Padding(
                      ...
                      child: Container(
                        padding: const EdgeInsets.all(12),
                        decoration: BoxDecoration(
                          color: AppColors.debit.withOpacity(0.05),
                          borderRadius: BorderRadius.circular(10),
                        ),
                        child: Column(
                          children: [
                            ...
                            if (product != null) ...[
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                    CurrencyFormatter.formatAmount(
                                      ((double.tryParse(_rateController.text) ?? 0) - product.purchasePrice) *
                                          (double.tryParse(_qtyController.text) ?? 0),
                                    ),
                                    style: const TextStyle(
                                      fontFamily: 'Roboto',
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.history, size: 16, color: AppColors.debit),
                ),
                const SizedBox(width: 8),
                const Text(
                  'حالیہ اسٹاک آؤٹ',
              ...
              data: (entries) {
                ...
                if (outEntries.isEmpty) {
                  return const EmptyStateWidget(
                    ...
                    title: 'ابھی کوئی اسٹاک آؤٹ نہیں',
                    description: 'اوپر فارم سے اسٹاک آؤٹ کریں',
                  );
                }
                return ListView.builder(
                  ...
                  itemCount: outEntries.length,
                  itemBuilder: (context, index) {
                    final entry = outEntries[index];
                    return _StockOutEntryCard(
                      entry: entry,
                      product: product!,
                      onDelete: () {
                        ref
...
class _StockOutEntryCard extends StatelessWidget {
  final StockEntryEntity entry;
  final ProductEntity product;
  final VoidCallback onDelete;

  const _StockOutEntryCard({
    required this.entry,
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.arrow_upward, color: AppColors.debit, size: 20),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
